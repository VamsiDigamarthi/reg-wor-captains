...
import { insuranceUpload } from "../Services/AadharCard.serv";
type docsImageType = {
  front: string | null;
  docFront: File | null;
};
...
const VehicleInsurance = () => {
  ...
  let colorType;
  switch (worUser?.services?.[0]?.insuranceVerification) {
    case "pending":
  ...
  let newFrontImage = worUser?.services?.[0]?.insuranceImg
  ...
  const handleUploadImage = async () => {
    ...
    setIsLoading(true);
    const success = await insuranceUpload(
      worUser?.mobile ?? "",
      images,
